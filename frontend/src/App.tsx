...
type BalanceTone = 'grid-import' | 'solar' | 'grid-export';

function BalanceValue({ value, total, language, tone, showShare = true }: { value: number | null | undefined; total?: number | null | undefined; language: Language; tone: BalanceTone; showShare?: boolean }) {
  ...
  return (
    <strong className={`balance-value ${tone}`}>
      ...
      {showShare && <span className="balance-value-separator" aria-hidden="true"> · </span>}
...
function SetupView({ onCurrentUserChange }: { onCurrentUserChange: (user: User) => void }) {
  ...
  return (
    <div className="grid gap setup-grid">
      <section className="setup-category setup-wide">
        <div className="setup-category-head"><h2><Settings size={20} /> {t('generalSettings')}</h2></div>
